...
// 获取分组短链接监控数据
func NewShortLinkGroupStatsLogic(ctx context.Context, svcCtx *svc.ServiceContext) *ShortLinkGroupStatsLogic {
	return &ShortLinkGroupStatsLogic{
...
func (l *ShortLinkGroupStatsLogic) ShortLinkGroupStats(req *types.ShortLinkGroupStatsReq) (resp *types.ShortLinkStatsRespDTO, err error) {
	...
	if err != nil {
		logx.Errorf("获取分组短链接统计数据失败: %v", err)
		return nil, err
	}
